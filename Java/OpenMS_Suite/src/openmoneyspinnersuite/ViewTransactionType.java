...
final class ViewTransactionType extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(openmoneyspinnersuite.OpenMS_Finance_App.class).getContext().getResourceMap(ViewTransactionType.class);
    ...
    private void jCancelButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCancelButtonActionPerformed
        if(processThread.isAlive())
        {
            ...
            this.setTitle("::. View Transaction Type");
            enableControls();
        }
        else
        dispose();
    ...
    {
        try{
            this.setTitle("::. View Transaction Type - Processing Request");
            thread.sleep(500);
            this.setTitle("::. View Transaction Type - Processing Request.");
            thread.sleep(500);
            this.setTitle("::. View Transaction Type - Processing Request..");
    ...
    {
        String[] transactionTypeCode=systems.getTableDataArrayNum(OpenMSApp.Database_A,"TransactionType",1);
        String[] transactionType=systems.getTableDataArrayNum(OpenMSApp.Database_A,"TransactionType",2);

        if(searchPara.isEmpty()==false)
        {
            String multipleSearch[]=searchPara.split(",");
            String queryPara="where (cTransactionTypeCode = '"+multipleSearch[0]+"') or (vTransactionType = '"+multipleSearch[0]+"') ";//or (cPhone like '%"+multipleSearch[0]+"%') ";

            for(int k=1;k<multipleSearch.length;k++)
            {
                queryPara+="or (cTransactionTypeCode like '%"+multipleSearch[k]+"%') or (vTransactionType like '%"+multipleSearch[k]+"%') ";//or (cPhone like '%"+multipleSearch[k]+"%') ";
            }

            transactionTypeCode=systems.getColumn(OpenMSApp.Database_A, "Select * from TransactionType "+queryPara, 1);
            transactionType=systems.getColumn(OpenMSApp.Database_A, "Select * from TransactionType "+queryPara, 2);
        }

        int x=0;
        int y=transactionTypeCode.length;

        tableObject=new Object[y][2];

        while(x<y)
        {
            tableObject[x][0]=transactionTypeCode[x].trim();
            tableObject[x][1]=transactionType[x].trim();
            systems.freeHeap();
            x+=1;
            if(x==y){break;}
       }
