...
final class ViewCurrency extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(openmoneyspinnersuite.OpenMS_Finance_App.class).getContext().getResourceMap(ViewCurrency.class);
    ...
    {
        try{
            this.setTitle("::. View Currency - Processing Request");
            thread.sleep(500);
            this.setTitle("::. View Currency - Processing Request.");
            thread.sleep(500);
            this.setTitle("::. View Currency - Processing Request..");
    ...
    {
        String[] currencyCode=systems.getTableDataArrayNum(OpenMSApp.Database_A,"Currency",1);
        String[] currencyName=systems.getTableDataArrayNum(OpenMSApp.Database_A,"Currency",2);

            if(searchPara.isEmpty()==false)
            {
                String multipleSearch[]=searchPara.split(",");
                String queryPara="where (cCurrencyCode = '"+multipleSearch[0]+"') or (vCurrencyName = '"+multipleSearch[0]+"') ";// or (cPhone like '%"+multipleSearch[0]+"%') ";

                for(int k=1;k<multipleSearch.length;k++)
                {
                    queryPara+="or (cCurrencyCode like '%"+multipleSearch[k]+"%') or (vCurrencyName like '%"+multipleSearch[k]+"%') ";// or (cPhone like '%"+multipleSearch[k]+"%') ";
                }

                currencyCode=systems.getColumn(OpenMSApp.Database_A, "Select * from Currency "+queryPara, 1);
                currencyName=systems.getColumn(OpenMSApp.Database_A, "Select * from Currency "+queryPara, 2);
            }

        int x=0;
        int y=currencyCode.length;

        tableObject=new Object[y][2];

        while(x<y)
        {
            tableObject[x][0]=currencyCode[x].trim();
            tableObject[x][1]=currencyName[x].trim();
            systems.freeHeap();
            x+=1;
            if(x==y){break;}
       }
