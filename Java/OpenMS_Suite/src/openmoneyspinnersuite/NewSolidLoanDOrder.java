...
final class NewSolidLoanDOrder extends javax.swing.JInternalFrame {
    ...
    String solidPayablesOrderCode,accountTyp,stockInValue;
    Thread processThread=new Thread();
    Thread titleThread=new Thread();

    /** Creates new form NewCreditOrder */
    public NewSolidLoanDOrder() {
        initComponents();
        setTitle("::. New Inventory Payable Debit Order By User ("+OpenMSApp.EmployeeName+")");
        reset();
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(openmoneyspinnersuite.OpenMS_Finance_App.class).getContext().getResourceMap(NewSolidLoanDOrder.class);
        ...
        jDebitAccountNameComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "-- Select --", "Item 2", "Item 3", "Item 4" }));
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            ...
                            .addComponent(jDebitAccountNameComboBox, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
    ...
    public void performMakeOrder(){
       ...
       int b=0;
        ...
        {
            JOptionPane.showMessageDialog(JOptionPane.getRootFrame(),"PLEASE SELECT INVENTORY PAYABLES ACCOUNT ","ERROR !",JOptionPane.ERROR_MESSAGE);
            return;
        }
        if(jDebitAccountNameComboBox.getSelectedItem().equals("-- Select --"))
        {
            JOptionPane.showMessageDialog(JOptionPane.getRootFrame(),"PLEASE SELECT DEBITED ACCOUNT ","ERROR !",JOptionPane.ERROR_MESSAGE);
            return;
        }
        String accountName=String.valueOf(jDebitAccountNameComboBox.getSelectedItem());
        ...
        {
            JOptionPane.showMessageDialog(JOptionPane.getRootFrame()," SELECTED INVENTORY PAYABLES ACCOUNT IS FROZEN ! \n SEE YOUR ACCOUNT ADMINISTRATOR FOR DETAILS.","ERROR !",JOptionPane.ERROR_MESSAGE);
            return;
        }
            String dActName=jSolidPayablesAccountNameComboBox.getSelectedItem().toString().trim();
            String cActName=jDebitAccountNameComboBox.getSelectedItem().toString().trim();

        if(jSolidPayablesAccountNameComboBox.getSelectedItem().equals("-- Select --")==false||jDebitAccountNameComboBox.getSelectedItem().equals("-- Select --")==false)
        {
        ...
        String debitAccountBalance=systems.getValue(OpenMSApp.Database_A, "Select * from Account where vAccountName='"+dActName+"'", "mAmount");
        ...
        {
                JOptionPane.showMessageDialog(JOptionPane.getRootFrame()," INSUFFICIENT FUND IN THE SELECTED INVENTROY PAYABLE ACCOUNT \n  FOR THIS TRANSACTION !","ERROR !",JOptionPane.ERROR_MESSAGE);
                return;
        }
        String debitAccountBalanceA=systems.getValue(OpenMSApp.Database_A, "Select * from Account where vAccountName='"+cActName+"'", "mAmount");
        String newAccountBalanceA=systems.getValue(OpenMSApp.Database_A, Connect.procInit+" prnSubtractMoney"+Connect.procInitStart+debitAccountBalanceA+","+jTotAmountTextField.getText().trim()+Connect.procInitEnd, "mSumValue");
        try
        {
            systems.converToRealMoneyFormat(newAccountBalanceA);
        }
        catch(Exception e)
        {
                JOptionPane.showMessageDialog(JOptionPane.getRootFrame()," INSUFFICIENT FUND IN THE SELECTED INVENTROY ACCOUNT \n  FOR THIS TRANSACTION !","ERROR !",JOptionPane.ERROR_MESSAGE);
                return;
        }
        }
        accountTyp=systems.getValue(OpenMSApp.Database_A, "Select * from Account where vAccountName='"+accountName+"'", "cCategory");
        if(accountTyp.equalsIgnoreCase("INVENTORY"))
        {
            stockInValue="IN";
            JOptionPane.showMessageDialog(JOptionPane.getRootFrame(),"DUE TO FIFO PRINCIPLES, CORRESPONDING INVENTORY ACCOUNT WILL AUTOMATICALLY BE DEBITED INDEPENDENTLY AFTER THIS TRANSACTION.","NOTICE !",JOptionPane.INFORMATION_MESSAGE);
        }           
        makeOrder();
        //reset();
        resetTable();
...
//                        }
    ...
    {
        String srActName=jDebitAccountNameComboBox.getSelectedItem().toString().trim();
        ...
        {
            jDebitActUnitTextField.setText(systems.getValue(OpenMSApp.Database_A, "Select * from Account Where vAccountName='"+srActName+"'","vAccountUnit" ));
        }
    }
...
private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if(processThread.isAlive())
        {
            ...
            this.setTitle("::. New Inventory Payable Debit Order By User ("+OpenMSApp.EmployeeName+")");
            enableControls();
        }
        else
            dispose();
    ...
    public void makeOrder()
    {
        ...
        String solidReceivablesAccountName=String.valueOf(jDebitAccountNameComboBox.getSelectedItem()).trim();
        String transactionType=String.valueOf(jTransactionTypeComboBox.getSelectedItem()).trim();
        String solidReceivablesOrderCode=systems.getValue(OpenMSApp.Database_A, Connect.procInit+" prnPrintDebitOrderCode"+Connect.procInitStart+Connect.procInitEnd, "cCode");

        String refCode=systems.getValue(OpenMSApp.Database_A, Connect.procInit+" prnPrintDebitRefID"+Connect.procInitStart+Connect.procInitEnd, "cCode");
        ...
        System.out.println("DebitOrdercode= "+solidReceivablesOrderCode);
...
//---
        ...
        solidPayablesOrderCode=systems.getValue(OpenMSApp.Database_A, Connect.procInit+" prnPrintDebitOrderCode"+Connect.procInitStart+Connect.procInitEnd, "cCode");
        ...
        System.out.println("DebitOrdercode= "+solidPayablesOrderCode);
        System.out.println("departmentcode= "+departmentCode);
        
        //Connect.createMSSQLConnection();
        Connect.changeDB(OpenMSApp.Database_A);
        /**--Please note: 
         * --Debit Inventory or expense account manually
         * */
        if(accountTyp.equalsIgnoreCase("EXPENSE"))
        {
            ...
            if(!jTable1.getValueAt(y,3).equals("0"))
            {
                 ...
                 try{
                        ...
                        statement.setString(3,jTable1.getValueAt(y,2).toString().trim()+" [Q="+jTable1.getValueAt(y,3)+", R="+jTable1.getValueAt(y,4)+"]");//--+systems.getValue(OpenMSApp.Database_A, "Select * from Item Where vName='"+jTable1.getValueAt(y,2)+"'", 4)
                    ...
                    {
                        System.out.println("NewSolidLoanDOrder2.makeOrder() gave error ! "+e);
                    }
                Connect.executeUpdateStatement(statement);
            }
            else
        ...
        statement=Connect.createStatement(Connect.procInit+"  prnInsDebitOrderOnOrder"+Connect.procInitStart+"?,?,?,?,?,?,?,?,?,?,?,?,?"+Connect.procInitEnd);
         try{
            ...
            {
                System.out.println("NewSolidLoanDOrder3.makeOrder() gave error ! "+e);
            }
        Connect.executeUpdateStatement(statement);
        ...
        while(yi<xi)
        {
            if(!jTable1.getValueAt(yi,3).equals("0"))
            {
             ...
             try{
                    ...
                    statement.setString(6,stockInValue);
                }
                catch(Exception e)
                {
                        System.out.println("NewSolidLoanDOrder4.makeOrder() gave error ! "+e);
                }
            Connect.executeUpdateStatement(statement);
            }
            else
    ...
    public void printVoucher()
    {
        ...
        try
        {
            ...
            note.writeVoucher(solidPayablesOrderCode,"",1);
            
            if(OpenMSApp.d==null)
            {
                OpenMSApp.d=new NoteViewer("Preview Voucher - "+solidPayablesOrderCode,Configuration.officialVoucherURLPath);
            ...
            else if(OpenMSApp.d.isVisible()==false)
            {
                ...
                OpenMSApp.d.setTitle("Preview Voucher - "+solidPayablesOrderCode);
                OpenMSApp.d.doTas(2);
    ...
    public void selectUserDefault()
    {
        if(jCheckBox4.isSelected()==false)
        {
            ...
            jDebitAccountNameComboBox.setSelectedItem("STOCK A/C NGN");
    ...
    {
        try{
            this.setTitle("::. New Inventory Payable Debit Order - Processing Request");
            thread.sleep(500);
            this.setTitle("::. New Inventory Payable Debit Order - Processing Request.");
            thread.sleep(500);
            this.setTitle("::. New Inventory Payable Debit Order - Processing Request..");
    ...
    {
        String[] actName=systems.getTableDataArray(OpenMSApp.Database_A,"vwExpenseInventoryAccount",2);
        actName[0]="-- Select --";
