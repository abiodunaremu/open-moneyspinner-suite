...
final class ViewItemLocation extends javax.swing.JInternalFrame {
    ...
    public ViewItemLocation() {
        initComponents();
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            tableObject,
            new String [] {
                "Item Location Code", "Name"
            }
        ) {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(openmoneyspinnersuite.OpenMS_Finance_App.class).getContext().getResourceMap(ViewItemLocation.class);
    ...
    private void jCancelButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCancelButtonActionPerformed
        ...
        if(processThread.isAlive())
        {
            ...
            this.setTitle("::. View Item Location");
            enableControls();
        }
        else
        dispose();
    ...
    {
        try{
            this.setTitle("::. View Item Location - Processing Request");
            thread.sleep(500);
            this.setTitle("::. View Item Location - Processing Request.");
            thread.sleep(500);
            this.setTitle("::. View Item Location - Processing Request..");
    ...
    {
        String[] itemLocationCode=systems.getTableDataArrayNum(OpenMSApp.Database_A,"ItemLocation",1);
        String[] itemLocationName=systems.getTableDataArrayNum(OpenMSApp.Database_A,"ItemLocation",2);

        if(searchPara.isEmpty()==false)
        {
            String multipleSearch[]=searchPara.split(",");
            String queryPara="where (cItemLocationCode = '"+multipleSearch[0]+"') or (vLocationName = '"+multipleSearch[0]+"') ";//or (cPhone like '%"+multipleSearch[0]+"%') ";

            for(int k=1;k<multipleSearch.length;k++)
            {
                queryPara+="or (cItemLocationCode like '%"+multipleSearch[k]+"%') or (vLocationName like '%"+multipleSearch[k]+"%') ";//or (cPhone like '%"+multipleSearch[k]+"%') ";
            }

            itemLocationCode=systems.getColumn(OpenMSApp.Database_A, "Select * from ItemLocation "+queryPara, 1);
            itemLocationName=systems.getColumn(OpenMSApp.Database_A, "Select * from ItemLocation "+queryPara, 2);
        }

        int x=0;
        int y=itemLocationCode.length;

        tableObject=new Object[y][2];

        while(x<y)
        {
            tableObject[x][0]=itemLocationCode[x].trim();
            tableObject[x][1]=itemLocationName[x].trim();
            systems.freeHeap();
            x+=1;
            if(x==y){break;}
       }
