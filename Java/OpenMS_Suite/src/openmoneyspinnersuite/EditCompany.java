...
final class EditCompany extends javax.swing.JInternalFrame {
    PreparedStatement statement;
    ResultSet result;
    Systems systems=new Systems();
    Thread editCompanyThread=new Thread();
    Thread editCompanyTitleThread=new Thread();
    ...
    /** Creates new form Customer */
     EditCompany() {
        ...
        editCompanyThread=new Thread(new Runnable() {
            public void run() {
                disableControls();
                resetCustomerID();
                resetCountry();
        ...
        editCompanyTitleThread=new Thread(new Runnable() {
            public void run() {
                while(editCompanyThread.isAlive())setProcessingTitle(editCompanyTitleThread);
                enableControls();
                System.out.println("accountThread stopped runing.");
            }
        });
        editCompanyThread.start();
        editCompanyTitleThread.start();
    }
    ...
    private void initComponents() {
        ...
        setClosable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(openmoneyspinnersuite.OpenMS_Finance_App.class).getContext().getResourceMap(EditCompany.class);
        ...
        customerIDComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "-- Select --" }));
        customerIDComboBox.setName("customerIDComboBox"); // NOI18N
    ...
    private void jUpdateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jUpdateButtonActionPerformed
        ...
        if(customerIDComboBox.getSelectedItem().equals("-- Select --"))
        {
            JOptionPane.showMessageDialog(JOptionPane.getRootFrame(),"Please Select a Company Code ","ERROR !",JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    {
        try{
            this.setTitle("::. Edit Company - Processing Request");
            thread.sleep(500);
            this.setTitle("::. Edit Company - Processing Request.");
            thread.sleep(500);
            this.setTitle("::. Edit Company - Processing Request..");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if(editCompanyThread.isAlive())
        {
            try{
                editCompanyThread.stop();
                editCompanyTitleThread.stop();
    ...
    private void customerIDSelected()
    {
        String customerID=customerIDComboBox.getSelectedItem().toString();
        if(customerID.equals("-- Select --"))
        {
            resetCustomerID();
            resetFields();
        }
        else
        {
            try
            {
                ...
                statement=Connect.createStatement("Select * from Company where cCompanyCode= '"+customerID+"'");
            ...
            {
                System.out.println("Sql errror! at EditCompany.customerIDComboBoxActionPerformed "+e);
            }

            dbCountry=systems.getValue(OpenMSApp.Database_C,"Select * from country where cCountryCode= '"+dbCountryCode+"'","vCountryName");
    ...
    {
        systems=new Systems();
        String[] customerID=systems.getTableDataArray(OpenMSApp.Database_C,"Company",1);
        customerID[0]="-- Select --";

        customerIDComboBox.setModel(new DefaultComboBoxModel(customerID));
        jCountryComboBox.setSelectedItem("NIGERIA");
    }
     void resetFields()
    {
     ...
     void submitData()
    {
        //Connect.createMSSQLConnection();
        Connect.changeDB(OpenMSApp.Database_C);
        statement=Connect.createStatement(Connect.procInit+"  prnUpdCompany"+Connect.procInitStart+"?,?,?,?,?,?,?,?"+Connect.procInitEnd);
            try{
            statement.setString(1,customerIDComboBox.getSelectedItem().toString());
            ...
            {
            System.out.println("EditCompany.submitData() gave error ! "+e);
            }
        Connect.alert="Y";
