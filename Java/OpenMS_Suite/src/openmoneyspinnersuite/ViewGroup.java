...
final class ViewGroup extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(openmoneyspinnersuite.OpenMS_Finance_App.class).getContext().getResourceMap(ViewGroup.class);
    ...
    {
        try{
            this.setTitle("::. View Group - Processing Request");
            thread.sleep(500);
            this.setTitle("::. View Group - Processing Request.");
            thread.sleep(500);
            this.setTitle("::. View Group - Processing Request..");
    ...
    {
        String[] groupCode=systems.getTableDataArrayNum(OpenMSApp.Database_A,"Groups",1);
        String[] groupName=systems.getTableDataArrayNum(OpenMSApp.Database_A,"Groups",2);

            if(searchPara.isEmpty()==false)
            {
                String multipleSearch[]=searchPara.split(" ");
                String queryPara="where (cGroupCode like '%"+multipleSearch[0]+"%') or (vName like '%"+multipleSearch[0]+"%') ";// or (cPhone like '%"+multipleSearch[0]+"%') ";

                for(int k=1;k<multipleSearch.length;k++)
                {
                    queryPara+="or (cGroupCode like '%"+multipleSearch[k]+"%') or (vName like '%"+multipleSearch[k]+"%') ";// or (cPhone like '%"+multipleSearch[k]+"%') ";
                }

                groupCode=systems.getColumn(OpenMSApp.Database_A, "Select * from Groups "+queryPara, 1);
                groupName=systems.getColumn(OpenMSApp.Database_A, "Select * from Groups "+queryPara, 2);
            }

        int x=0;
        int y=groupCode.length;

        tableObject=new Object[y][2];

        while(x<y)
        {
            tableObject[x][0]=groupCode[x].trim();
            tableObject[x][1]=groupName[x].trim();
            systems.freeHeap();
            x+=1;
            if(x==y){break;}
       }
